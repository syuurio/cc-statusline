...
export function generateScript(config) {
  ...
  // ── Preamble ────────────────────────────────────────────────────────────
  push(`#!/bin/bash`)
  push(`# cc-statusline — generated by cc-statusline wizard`)
  ...
  if (needsRate) {
    ...
    push(`CACHE_MAX_AGE=300`)
    push(`LOCK_MAX_AGE=30`)
    push(``)
  }
  ...
  if (has(fields, 'resetTimes')) {
    ...
    push(`five_reset_fmt=$(format_reset_time "\${five_hour_reset:-}" "%H:%M")`)
    push(`seven_reset_fmt=$(format_reset_time "\${seven_day_reset:-}" "%-m/%-d %H:%M")`)
  }
  push(``)
  ...
  if (has(fields, 'resetTimes')) {
    ...
    push(`line3+="\${C_GRAY}╰\${C_RESET}"`)
    push(`if [[ -n "$five_hour_pct" ]] && [[ "$five_hour_pct" != "null" ]]; then`)
    push(`    line3+=" \${C_GRAY}5-hour resets \${five_reset_fmt:-—}\${C_RESET}"`)
    push(`    line3+="\${SEP}\${C_GRAY}weekly resets \${seven_reset_fmt:-—}\${C_RESET}"`)
    push(`fi`)
    push(``)
  }
